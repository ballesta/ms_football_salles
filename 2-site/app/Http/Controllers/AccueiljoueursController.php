...
class AccueiljoueursController extends Controller {
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['subgrid']	= (isset($this->info['config']['subgrid']) ? $this->info['config']['subgrid'] : array()); 
			$this->data['fields'] =  \AjaxHelpers::fieldLang($this->info['config']['grid']);
			return view('accueiljoueurs.view',$this->data);
		} else {
			return Redirect::to('accueiljoueurs')->with('messagetext','Record Not Found !')->with('msgstatus','error');					
		}
	}
	...
	function postCopy( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			\DB::select($sql);
			return Redirect::to('accueiljoueurs')->with('messagetext',\Lang::get('core.note_success'))->with('msgstatus','success');
		} else {
		
			return Redirect::to('accueiljoueurs')->with('messagetext','Please select row to copy')->with('msgstatus','error');
		}	
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			// redirect
			return Redirect::to('accueiljoueurs')
