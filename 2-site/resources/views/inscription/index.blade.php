...
<div class="col-md-6">
@if($access['is_add'] ==1)
<a href="{{ URL::to('inscription/update') }}" class="tips btn btn-sm btn-white"  title="{{ Lang::get('core.btn_create') }}">
...
@endif
<a href="{{ URL::to( 'inscription/search') }}" class="btn btn-sm btn-white" onclick="SximoModal(this.href,'Advance Search'); return false;" title="{{ Lang::get('core.btn_search') }}"><i class="fa fa-search"></i> {{ Lang::get('core.btn_search') }} </a>
</div>
<div class="col-md-6 ">
...
{!! Form::open(array('url'=>'inscription/delete/', 'class'=>'form-horizontal' ,'id' =>'SximoTable' )) !!}
...
$(document).ready(function(){
    ...
    $('#SximoTable').attr('action','{{ URL::to("inscription/multisearch")}}');
        $('#SximoTable').submit();
    });
    $('.copy').click(function() {
        ...
        {
        $('#SximoTable').attr('action','{{ url("inscription/copy")}}');
